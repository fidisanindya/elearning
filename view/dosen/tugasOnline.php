        ...
        <div class="content">
            <div style="padding: 30px 20px">
                <div class="jumbotron" style="background-color: #fff">
                    ...
                    <div class="row">
                        <?php
                        $matkul = mysqli_query($connection, "SELECT distinct matkul_id, kelas_id FROM jadwal WHERE dosen_id='$result[user_id]'");
                        while ($vmatkul = mysqli_fetch_assoc($matkul)) {
                            ...
                            $vKelas = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM kelas WHERE id=$vmatkul[kelas_id]"));
                        ?>
                            <div class="col-md-4">
                                <div class="card" style="margin-top: 20px; border-color: #748bdb">
                                    <div style="margin: 20px 20px 0px 20px;">
                                        <p style="font-weight: bold; color:  #575555"><?php echo $vMatkul['matkul'] ?></p>
                                        <p style="margin-top: -10px;"><?php echo $vKelas['kelas'] ?></p>
                                        <a href="daftarTugas.php?matkul=<?php echo $vmatkul['matkul_id']; ?>&kelas=<?php echo $vmatkul['kelas_id'] ?>" class="btn btn-primary" style="margin-bottom: 15px; margin-top: 10px">Detail</a>
                                    </div>
                                </div>
                            </div>
                        <?php
