                ...
                <div class="jumbotron" style="background-color: #fff">
                    ...
                    <div class="row">
                        ...
                        for ($i = 0; $i < count($hari); $i++) { ?>
                            <div class="col-md-4">
                                <div class="card" style="height: 370px; margin-top: 30px">
                                    <h5 class="card-header" style="text-align: center; background-color: #2a34bf; color: white"><?php echo $hari[$i] ?></h5>
                                    <?php
                                    $jadwal = mysqli_query($connection, "SELECT * FROM jadwal WHERE kelas_id='$mahasiswa[kelas_id]' AND hari='$hari[$i]'");
                                    ...
                                    foreach ($jadwal as $vjadwal) {
                                        ...
                                        $dosen = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM dosen WHERE id = $vjadwal[dosen_id]"));
                                    ?>
                                        <div style="margin: 20px 20px 0px 20px;">
                                            <p style="font-weight: bold"><?php echo $matkul['matkul'] ?></p>
                                            <p style="margin-top: -10px;"><i class="fas fa-user" style="margin-right: 10px"></i>Dosen : <?php echo $dosen['nama_depan'] . ' ' . $dosen['nama_belakang'] ?></p>
                                            <p style="margin-top: -10px;"><i class="fas fa-clock" style="margin-right: 10px"></i>Waktu : <?php echo $vjadwal['jam_mulai'] . ' - ' . $vjadwal['jam_selesai'] ?></p>
                                        </div>
                                    <?php } ?>
                                </div>
                            </div>
